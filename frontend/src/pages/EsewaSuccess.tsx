...
export const EsewaSuccess: React.FC = () => {
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const { confirmEsewaPayment, error } = useOrderStore();
  ...
  useEffect(() => {
    const verify = async () => {
      if (!dataParam) {
        setStatus('error');
        setErrorMessage('No transaction response data received from eSewa.');
        return;
      }

      try {
        const success = await confirmEsewaPayment(dataParam);
        if (success) {
          setStatus('success');
        } else {
          setStatus('error');
      ...
      <section className="glass" style={{ padding: '48px', borderRadius: '12px', textAlign: 'center', maxWidth: '500px', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        {status === 'verifying' && (
          <>
            ...
            <h2 style={{ fontSize: '24px', fontWeight: 800, marginBottom: '12px' }}>Verifying eSewa Payment</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '14px', lineHeight: 1.6 }}>
              Please wait while we verify your payment status with the eSewa servers. Do not refresh this page.
            </p>
          </>
        )}
        ...
        {status === 'success' && (
          <>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginBottom: '32px', lineHeight: 1.6 }}>
              Your eSewa payment was successfully processed. Your order status is now updated to processing.
            </p>
        ...
        {status === 'error' && (
          <>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '13px', marginBottom: '32px', lineHeight: 1.6 }}>
              If you believe this is an error and your money was deducted, please contact support with your transaction reference.
            </p>
